...
def extract_spectrum_pca(part):
    ...
    if not os.path.exists('C:/project1'+part+'/spectrum2/'): # 'spectrum' 디렉토리가 존재하지 않으면 만들어 줍니다.
        os.mkdir('C:/project1/'+part+'/spectrum2/')
    for f in sample_files:
        ...
        pca = PCA(n_components=24, whiten=True , random_state=0)
        pca.fit(S)
        x_train_pca = pca.transform(S)
        if part == 'train': # 'train'인 경우 합계와 제곱의 합을 누적합니다.
            data_sum += x_train_pca
            data_squared_sum += x_train_pca**2
        numpy.save('C:/project1/'+part+'/spectrum2/'+f+'.npy', x_train_pca) # 현재 샘플의 스펙트럼을 저장합니다.
    if part == 'train': # 모든 파일의 변환이 끝난 후에, 'train'인 경우 평균과 표준편차를 저장합니다.
        ...
        numpy.save('C:/project1/'+part+'/spectrum2/data_mean.npy', data_mean)
        numpy.save('C:/project1/'+part+'/spectrum2/data_std.npy', data_std)
        print("finish!!!")
